...
// RunCNN2L1Caching benchmarks normal vs single-core-tiled vs multi-core-tiled
// 2D forward pass across all numeric types (CPU only).
func RunCNN2L1Caching() {
	fmt.Println("=== CNN2 Multi-Core L1 Caching — All Numerical Types ===")
	iterations := 3
	...
	run := func(cfg typeConfig) result {
		ws := poly.NewWeightStore(32 * 32 * 3 * 3)
		...
		l := poly.VolumetricLayer{
			...
			Type:          poly.LayerCNN2,
			InputChannels: 32, InputHeight: 32, InputWidth: 32,
			Filters: 32, OutputHeight: 32, OutputWidth: 32,
			KernelSize: 3, Stride: 1, Padding: 1,
			DType: cfg.dtype, WeightStore: ws,
		}
		input := poly.NewTensor[float32](1, 32, 32, 32)
		...
		for i := 0; i < iterations; i++ {
			_, post0 = poly.CNN2ForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post1 = poly.CNN2ForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post2 = poly.CNN2ForwardPolymorphic(&l, input)
		}
...
// RunCNN2Training tests 6 training modes × 21 numeric types (train + save/reload).
func RunCNN2Training() {
	const (
		batchSz             = 1
		inC                 = 4
		inH, inW            = 4, 4
	...
	buildNet := func(dtype poly.DType, scale float32) (*poly.VolumetricNetwork, error) {
		spec := netSpec{
			ID: "cnn2_train_test", Depth: 1, Rows: 1, Cols: 1, LayersPerCell: 1,
			Layers: []layerSpec{{
				Type: "CNN2", Activation: "LINEAR", DType: "FLOAT32",
				InputChannels: inC, InputHeight: inH, InputWidth: inW,
				Filters: filters, OutputHeight: outH, OutputWidth: outW,
	...
	makeBatch := func() poly.TrainingBatch[float32] {
		inp := poly.NewTensor[float32](batchSz, inC, inH, inW)
		for i := range inp.Data {
			inp.Data[i] = float32(i%13)*0.1 - 0.6
		}
		tgt := poly.NewTensor[float32](batchSz, filters, outH, outW)
	...
	saveReload := func(net *poly.VolumetricNetwork) (*poly.VolumetricNetwork, int, error) {
		...
		tmp, err := os.CreateTemp("", "poly_cnn2_*.json")
		if err != nil {
			return nil, 0, err
		}
		path := tmp.Name()
	...
	fmt.Println("=== CNN2 Training — All Modes × All Numerical Types ===")
	fmt.Println()

	testNet := poly.NewVolumetricNetwork(1, 1, 1, 1)
	gpuAvail := testNet.InitWGPU() == nil
	if gpuAvail {
		sc, mc := poly.CNN2GPUTileSizes(testNet.GPUContext)
...
// RunCNN2GPUForward compares CPU multi-core tiled vs GPU Normal/SC/MC forward pass.
func RunCNN2GPUForward() {
	fmt.Println("=== CNN2 GPU Forward — All Numerical Types ===")
	fmt.Println()
	...
	ctx := net.GPUContext
	scTile, mcTile := poly.CNN2GPUTileSizes(ctx)
	...
	const (
		inC, inH, inW      = 32, 32, 32
		outC, outH, outW   = 32, 32, 32
		kH, kW             = 3, 3
		sH, sW             = 1, 1
		pH, pW             = 1, 1
		batchSize          = 1
		outputSize         = batchSize * outC * outH * outW
		kernelVol          = inC * kH * kW // 288
	)
	...
	run := func(cfg typeConfig) result {
		...
		l := poly.VolumetricLayer{
			...
			Type:          poly.LayerCNN2,
			InputChannels: inC, InputHeight: inH, InputWidth: inW,
			Filters: outC, OutputHeight: outH, OutputWidth: outW,
			KernelSize: kH, Stride: 1, Padding: 1,
		...
		input := poly.NewTensor[float32](batchSize, inC, inH, inW)
		for i := range input.Data {
			input.Data[i] = 0.5
		}
		...
		start := time.Now()
		for i := 0; i < iterations; i++ {
			_, cpuMC = poly.CNN2ForwardPolymorphic(&l, input)
		}
		...
		// Warmup
		ctx.DispatchCNN2Scaled(batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, cfg.scale, inputBuf, rawWeightBuf, outputBuf)
		ctx.DispatchCNN2Tiled(scTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, cfg.scale, inputBuf, rawWeightBuf, outputBuf)
		ctx.DispatchCNN2Tiled(mcTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, cfg.scale, inputBuf, rawWeightBuf, outputBuf)
...
// RunCNN2GPUBackward compares CPU multi-core tiled vs GPU Normal/SC/MC backward pass.
func RunCNN2GPUBackward() {
	fmt.Println("=== CNN2 GPU Backward — All Numerical Types ===")
	fmt.Println()
	...
	run := func(cfg typeConfig) result {
		...
		gradOut := poly.NewTensor[float32](batchSize, outC, outH, outW)
		...
		preAct := poly.NewTensor[float32](batchSize, outC, outH, outW)
		...
		start := time.Now()
		for i := 0; i < 3; i++ {
			cpuDX, cpuDW = poly.CNN2BackwardTiledParallel(&l, gradOut, input, preAct)
		}
		...
		// Warmup
		{
			...
			ctx.DispatchCNN2BackwardDX(batchSize, inC, inH, inW, outC, outH, outW, kSize, stride, padding, act, gradOutBuf, weightBuf, preActBuf, gi)
			ctx.DispatchCNN2BackwardDW(batchSize, inC, inH, inW, outC, outH, outW, kSize, stride, padding, act, gradOutBuf, inputBuf, preActBuf, gw)
			ctx.DispatchCNN2TiledBackwardDX(scTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, weightBuf, preActBuf, gi)
			ctx.DispatchCNN2TiledBackwardDW(scTile, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, inputBuf, preActBuf, gw)
			ctx.DispatchCNN2TiledBackwardDX(mcTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, weightBuf, preActBuf, gi)
			ctx.DispatchCNN2TiledBackwardDW(mcTile, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, inputBuf, preActBuf, gw)
			ctx.Device.Poll(true, nil)
			gi.Destroy()
			gw.Destroy()
		}
		...
		for i := 0; i < gpuIters; i++ {
			t0 := time.Now()
			ctx.DispatchCNN2BackwardDX(batchSize, inC, inH, inW, outC, outH, outW, kSize, stride, padding, act, gradOutBuf, weightBuf, preActBuf, timingGI)
			ctx.DispatchCNN2BackwardDW(batchSize, inC, inH, inW, outC, outH, outW, kSize, stride, padding, act, gradOutBuf, inputBuf, preActBuf, timingGW)
			ctx.Device.Poll(true, nil)
		...
		for i := 0; i < gpuIters; i++ {
			t0 := time.Now()
			ctx.DispatchCNN2TiledBackwardDX(scTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, weightBuf, preActBuf, timingGI)
			ctx.DispatchCNN2TiledBackwardDW(scTile, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, inputBuf, preActBuf, timingGW)
			ctx.Device.Poll(true, nil)
		...
		for i := 0; i < gpuIters; i++ {
			t0 := time.Now()
			ctx.DispatchCNN2TiledBackwardDX(mcTile, kernelVol, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, weightBuf, preActBuf, timingGI)
			ctx.DispatchCNN2TiledBackwardDW(mcTile, batchSize, inC, inH, inW, outC, outH, outW, kH, kW, sH, sW, pH, pW, act, gradOutBuf, inputBuf, preActBuf, timingGW)
			ctx.Device.Poll(true, nil)
