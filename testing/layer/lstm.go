...
// RunLSTML1Caching benchmarks normal vs single-core-tiled vs multi-core-tiled
// LSTM forward pass across all numeric types (CPU only).
func RunLSTML1Caching() {
	fmt.Println("=== LSTM Multi-Core L1 Caching — All Numerical Types ===")
	iterations := 3
	...
	// per gate = hiddenSize*(inputSize+hiddenSize+1); total = 4*gate
	const gateSize = hiddenSize*(inputSize+hiddenSize+1)
	const wCount = 4 * gateSize
	...
	run := func(cfg typeConfig) result {
		...
		for i := 0; i < iterations; i++ {
			_, post0 = poly.LSTMForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post1 = poly.LSTMForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post2 = poly.LSTMForwardPolymorphic(&l, input)
		}
...
// RunLSTMTraining tests 3 CPU training modes × 21 numeric types (train + save/reload).
func RunLSTMTraining() {
	const (
		batchSz    = 1
		inputSize  = 16
		hiddenSize = 32
	...
	buildNet := func(dtype poly.DType, scale float32) (*poly.VolumetricNetwork, error) {
		spec := netSpec{
			ID: "lstm_train_test", Depth: 1, Rows: 1, Cols: 1, LayersPerCell: 1,
			Layers: []layerSpec{{
				Type: "LSTM", Activation: "LINEAR", DType: "FLOAT32",
	...
	saveReload := func(net *poly.VolumetricNetwork) (*poly.VolumetricNetwork, int, error) {
		...
		tmp, err := os.CreateTemp("", "poly_lstm_*.json")
		if err != nil {
			return nil, 0, err
		}
		path := tmp.Name()
	...
	fmt.Println("=== LSTM Training — All Modes × All Numerical Types ===")
	fmt.Println()
...
// RunLSTMGPUForward checks GPU availability and reports that GPU kernels for LSTM
// are not yet implemented.
func RunLSTMGPUForward() {
	fmt.Println("=== LSTM GPU Forward — All Numerical Types ===")
	fmt.Println()
	...
	fmt.Println("GPU kernels for LSTM are not yet implemented.")
	fmt.Println("This test is a placeholder for future GPU support.")
}

// RunLSTMGPUBackward checks GPU availability and reports that GPU kernels for LSTM
// are not yet implemented.
func RunLSTMGPUBackward() {
	fmt.Println("=== LSTM GPU Backward — All Numerical Types ===")
	fmt.Println()
	net := poly.NewVolumetricNetwork(1, 1, 1, 1)
	if err := net.InitWGPU(); err != nil {
		fmt.Printf("GPU init failed: %v\nThis test requires a WebGPU-capable GPU.\n", err)
