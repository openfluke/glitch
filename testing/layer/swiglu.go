...
// RunSwiGLUL1Caching benchmarks normal vs single-core-tiled vs multi-core-tiled
// SwiGLU forward pass across all numeric types (CPU only).
func RunSwiGLUL1Caching() {
	fmt.Println("=== SwiGLU Multi-Core L1 Caching — All Numerical Types ===")
	iterations := 3
	...
	run := func(cfg typeConfig) result {
		...
		for i := 0; i < iterations; i++ {
			_, post0 = poly.SwiGLUForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post1 = poly.SwiGLUForwardPolymorphic(&l, input)
		}
		...
		start = time.Now()
		for i := 0; i < iterations; i++ {
			_, post2 = poly.SwiGLUForwardPolymorphic(&l, input)
		}
...
func RunSwiGLUTraining() {
	...
	buildNet := func(dtype poly.DType, scale float32) (*poly.VolumetricNetwork, error) {
		spec := netSpec{
			ID: "swiglu_train_test", Depth: 1, Rows: 1, Cols: 1, LayersPerCell: 1,
			Layers: []layerSpec{{
				Type: "SWIGLU", Activation: "LINEAR", DType: "FLOAT32",
				InputHeight: inputSize, OutputHeight: intermediateSize,
			}},
		}
	...
	makeBatch := func() poly.TrainingBatch[float32] {
		inp := poly.NewTensor[float32](seqLen, inputSize)
	...
	saveReload := func(net *poly.VolumetricNetwork) (*poly.VolumetricNetwork, int, error) {
		...
		tmp, err := os.CreateTemp("", "poly_swiglu_*.json")
		if err != nil {
			return nil, 0, err
		}
		path := tmp.Name()
	...
	fmt.Println("=== SwiGLU Training — All Modes × All Numerical Types ===")
	fmt.Println()
