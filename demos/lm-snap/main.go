...
func main() {
	...
	fmt.Println("║  Tiny Shakespeare  ·  character-level  ·  no gradient descent  ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════════╝")
	fmt.Println()
	...
	test := corpus[valEnd:]
	fmt.Printf("[*] Corpus: %d chars  |  vocab: %d unique chars\n", n, V)
	fmt.Printf("[*] Split:  train=%d  |  val=%d  |  test=%d\n\n", len(train), len(val), len(test))

	// ── 2. BUILD NETWORK ──
	// Architecture: Sequential([KMeans(V→V), Dense(V→V, linear)])
	// KMeans finds the nearest "context cluster" for the previous character.
	// Dense maps the cluster assignment to a probability distribution over next chars.
	...
	// Soft assignment to nearest char cluster, then transition probabilities.
	// The identity KMeans maps each one-hot input to "I'm character i" sharply.
	installIdentityKMeans := func(temp float64) {
		kmeansL.NumClusters = V
		kmeansL.InputHeight = V
		kmeansL.OutputHeight = V
		kmeansL.KMeansTemperature = temp
		kmeansL.WeightStore = poly.NewWeightStore(V * V)
		for i := 0; i < V; i++ {
			kmeansL.WeightStore.Master[i*V+i] = 1.0 // center i = one-hot e_i
		}
	}

	// Dense weight layout: W[j*V+i] → output[j] += input[i] * W[j*V+i]
	// So W[j*V+i] = P(next=j | cluster=i)
	installDenseWeights := func(W []float32) {
		denseL.InputHeight = V
		denseL.OutputHeight = V
		denseL.Activation = poly.ActivationLinear
		denseL.WeightStore = poly.NewWeightStore(V * V)
		copy(denseL.WeightStore.Master, W)
	}
	...
	evalPPL := func(corpus []int) float64 {
		return evalPerplexity(net, corpus, V)
	}

	rng := rand.New(rand.NewSource(42))
	seedChar := train[100] // start generation mid-corpus (avoids BOS issues)
	...
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	fmt.Println("  BASELINE  —  random weights (network just built, no statistics)")
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	...
	printWrapped(generateText(net, seedChar, vocab, V, GenLength, 1.0, rng))
	fmt.Println()
	...
	fmt.Println("           Action: install log-unigram as Dense weights (logits).")
	fmt.Println("           Each Dense column = same log-freq table (context-free).")
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	t0 := time.Now()
	unigram := computeUnigram(train, V)
	// Dense stores LOG-probabilities: softmax(log P[j]) = P[j]/ΣP = P[j]
	// This means the network outputs logits that softmax correctly recovers.
	// Each column = log unigram distribution (same regardless of prev char = context-free)
	uniW := make([]float32, V*V)
	for j := 0; j < V; j++ {
		logP := float32(math.Log(float64(unigram[j])))
		for i := 0; i < V; i++ {
			uniW[j*V+i] = logP
		}
	}
	installIdentityKMeans(0.1) // temperature doesn't matter (all columns identical)
	installDenseWeights(uniW)
	...
	printWrapped(generateText(net, seedChar, vocab, V, GenLength, 1.0, rng))
	fmt.Println()
	history = append(history, GenResult{"Unigram", pplxUnigram})
	...
	fmt.Println("           Action: install log bigram P(next|prev) as Dense weights (logits).")
	fmt.Println("           KMeans centers = identity (one-hot per char) at T=0.1.")
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	t0 = time.Now()
	bigram := computeBigram(train, V)
	// bigramW[j*V+i] = log P(next=j | prev=i) — log-probs as logits
	// softmax(Dense output) then recovers the exact bigram probabilities
	bigramW := make([]float32, V*V)
	for i := 0; i < V; i++ {
		for j := 0; j < V; j++ {
			bigramW[j*V+i] = float32(math.Log(float64(bigram[i][j])))
		}
	}
	installIdentityKMeans(0.1)
	installDenseWeights(bigramW)
	...
	printWrapped(generateText(net, seedChar, vocab, V, GenLength, 1.0, rng))
	fmt.Println()
	history = append(history, GenResult{"Bigram (T=0.10)", pplxBigram})
	...
	// Keep bigram at best temperature for comparison
	installIdentityKMeans(bestT)
	installDenseWeights(bigramW)
	history = append(history, GenResult{fmt.Sprintf("Bigram (T=%.2f)", bestT), bestPPL})

	// ── GEN 4: TRIGRAM (double the context window) ──
	// Input: concat(one-hot(c_{t-2}), one-hot(c_{t-1})) = 2V = 130 floats
	// KMeans: V² = 4225 clusters, one center per (c_{t-2}, c_{t-1}) pair
	//   Distance 0  = exact trigram match → sharp lookup
	//   Distance 2  = one char matches    → bigram-style fallback
	//   Distance 4  = both chars differ   → near-zero probability
	// Dense: V² → V, log-probs of P(next | c_{t-2}, c_{t-1})
	K := V * V // number of bigram contexts
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	fmt.Println("  [GEN 4]  Hypothesis: doubling the context window improves predictions.")
	fmt.Printf("           Action: 2-char context → %d KMeans clusters (%d²), 2×%d input.\n", K, V, V)
	fmt.Println("           Centers = concat(one-hot(c1), one-hot(c2)).  Dense = log-trigram.")
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	t0 = time.Now()

	trigram := computeTrigram(train, V)

	// KMeans centers: K centers, each 2V-dim = [one-hot(c1) | one-hot(c2)]
	...
	for c1 := 0; c1 < V; c1++ {
		for c2 := 0; c2 < V; c2++ {
			k := c1*V + c2
			kmeansL.WeightStore.Master[k*2*V+c1] = 1.0   // one-hot for c1
			kmeansL.WeightStore.Master[k*2*V+V+c2] = 1.0 // one-hot for c2
		}
	}

	// Dense: K inputs → V outputs, log-prob weights
	trigramW := make([]float32, V*K)
	for k := 0; k < K; k++ {
		for j := 0; j < V; j++ {
			trigramW[j*K+k] = float32(math.Log(float64(trigram[k][j])))
		}
	}
	...
	pplxTrigram := evalPerplexityTrigram(net, valEval, V)
	...
	seed2 := train[101]
	printWrapped(generateTextTrigram(net, seedChar, seed2, vocab, V, GenLength, 1.0, rng))
	fmt.Println()
	history = append(history, GenResult{"Trigram (2-char ctx)", pplxTrigram})

	// ── TEST SET ──
	fmt.Println("[*] Final evaluation on held-out test set (trigram)...")
	pplxTest := evalPerplexityTrigram(net, testEval, V)
	fmt.Printf("    Test perplexity: %.4f\n\n", pplxTest)

	// ── TEXT GENERATION SHOWCASE ──
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	fmt.Println("  TEXT GENERATION  —  trigram model (2-char context)")
	fmt.Println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	for _, sTemp := range []float64{0.5, 1.0, 1.5} {
		fmt.Printf("\n  Sampling temperature %.1f:\n", sTemp)
		printWrapped(generateTextTrigram(net, seedChar, seed2, vocab, V, GenLength, sTemp, rng))
	}
	fmt.Println()
	...
	fmt.Println("╠══════════════════════════════════════════════════╬══════════════╣")
	fmt.Printf("║ %-48s ║  %8.2f    ║\n", "FINAL TEST SET (trigram)", pplxTest)
	...
	for _, r := range history {
		...
		fmt.Printf("  %-24s  ppl=%6.2f  %s\n", r.name, r.valPPL, bar)
	}
	fmt.Printf("  %-24s  ppl=%6.2f  (test)\n\n", "Trigram final", pplxTest)
}

// ── NETWORK CONSTRUCTION ────────────────────────────────────────────────────

func buildNetwork(V int) *poly.VolumetricNetwork {
	// Sequential: KMeans(V→V) identifies the previous character cluster,
	// Dense(V→V) maps it to a probability distribution over next chars.
	jsonStr := fmt.Sprintf(`{
		"id": "lm_snap",
	...
	if err != nil {
		panic(fmt.Sprintf("buildNetwork: %v", err))
	}
	return net
}
...
// computeTrigram returns trigram[c1*V+c2][next] = P(next | c1, c2) with +1 smoothing.
...
// evalPerplexityTrigram evaluates perplexity with 2-char context (2V-dim input).
func evalPerplexityTrigram(net *poly.VolumetricNetwork, corpus []int, V int) float64 {
	if len(corpus) < 3 {
		return math.Inf(1)
	}
	totalLogProb := 0.0
	...
	for i := 2; i < len(corpus); i++ {
		...
		input.Data[corpus[i-2]] = 1.0       // one-hot of c_{t-2}
		input.Data[V+corpus[i-1]] = 1.0     // one-hot of c_{t-1} (offset by V)
...
// generateTextTrigram generates text using a 2-char context window.
func generateTextTrigram(net *poly.VolumetricNetwork, seed1, seed2 int, vocab []byte, V, length int, sampTemp float64, rng *rand.Rand) string {
...
// computeUnigram returns P(char) with +1 Laplace smoothing.
...
// computeBigram returns bigram[prev][next] = P(next|prev) with +1 smoothing.
func computeBigram(corpus []int, V int) [][]float32 {
	...
	for i := range bigram {
		...
		total := V // smoothing adds V to denominator
...
// evalPerplexity computes exp(-mean log P(next|prev)) over the corpus.
// Lower is better. Random baseline ≈ vocab size.
func evalPerplexity(net *poly.VolumetricNetwork, corpus []int, V int) float64 {
	if len(corpus) < 2 {
		return math.Inf(1)
	}
	totalLogProb := 0.0
...
func generateText(net *poly.VolumetricNetwork, seed int, vocab []byte, V, length int, sampTemp float64, rng *rand.Rand) string {
...
// pplxBar shows improvement: longer bar = lower perplexity (better).
